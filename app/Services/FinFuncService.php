 ...
 * @author    Enrique Garcнa M. <egarcia@egm.co>
 * @copyright (c) 2003-2012 Enrique Garcнa M.
...
class FinFuncService
{
     ...
     * @param string $date a Unix timestamp value.
     * @return float|int a Unix timestamp.
     */
    public function DATEADD(string $datepart, int $number, string $date): float|int
    {
        $number = (int)$number;
        switch (strtolower($datepart)) {
            ...
            case 'year':
                $d = getdate($date);
                $d['year'] += $number;
                if (((int)$d['mday'] === 29) && ((int)$d['mon'] === 2) && (date('L', mktime(0, 0, 0, 1, 1, $d['year'])) == 0)) $d['mday'] = 28;
     ...
     */
    public function DATEDIFF($datepart, $startdate, $enddate): float|int
    {
        switch (strtolower($datepart)) {
            case 'yy':
            case 'yyyy':
     ...
     * @return bool
     */
    public function _is_valid_basis(int $basis): bool
    {
     ...
     * @return bool
     */
    public function _is_valid_frequency(int $frequency): bool
    {
     ...
     */
    public function DAYS360($start_date, $end_date, $method = false): float|int
    {
        if ($method) {
     ...
     */
    public function Thirty360USdayCount($startdate, $enddate): float|int
    {
     ...
     */
    public function Thirty360USyearFraction($startdate, $enddate): float
    {
